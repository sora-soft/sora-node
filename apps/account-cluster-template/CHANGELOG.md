# @sora-soft/account-cluster-template

## 1.2.7

### Patch Changes

- 82b9396: WebSocketListener 构造函数新增 koa 参数用于在同端口处理普通 http 请求（如健康检查），并暴露 httpServer getter；修复 shutdown 时未显式关闭 httpServer 导致端口泄漏的问题。模板中 ws listener 与 http listener 共享同一个 koa 实例
- Updated dependencies [82b9396]
  - @sora-soft/http-support@2.3.0

## 1.2.6

### Patch Changes

- Updated dependencies [7ea31c3]
  - @sora-soft/etcd-discovery@2.2.5

## 1.2.5

### Patch Changes

- Updated dependencies [bf53f5a]
- Updated dependencies [92bd677]
  - @sora-soft/etcd-discovery@2.2.4
  - @sora-soft/etcd-component@2.2.4

## 1.2.4

### Patch Changes

- Updated dependencies [3ba31ed]
  - @sora-soft/http-support@2.2.3

## 1.2.3

### Patch Changes

- Updated dependencies
  - @sora-soft/framework@2.2.2
  - @sora-soft/database-component@2.2.2
  - @sora-soft/etcd-component@2.2.3
  - @sora-soft/etcd-discovery@2.2.3
  - @sora-soft/http-support@2.2.2
  - @sora-soft/redis-component@2.2.2

## 1.2.2

### Patch Changes

- Updated dependencies
  - @sora-soft/framework@2.2.1
  - @sora-soft/database-component@2.2.1
  - @sora-soft/etcd-component@2.2.2
  - @sora-soft/etcd-discovery@2.2.2
  - @sora-soft/http-support@2.2.1
  - @sora-soft/redis-component@2.2.1

## 1.2.1

### Patch Changes

- 656d8b1: feat: 修改项目模板中的问题
- Updated dependencies [656d8b1]
  - @sora-soft/etcd-component@2.2.1
  - @sora-soft/etcd-discovery@2.2.1

## 1.2.0

### Minor Changes

- 3eb0584: 放弃 vite 编译链，使用 ts-patch + jest 测试平台
- 7bed083: 重新使用 ts-patch 作为编译器

### Patch Changes

- Updated dependencies [ae9abda]
- Updated dependencies [3eb0584]
- Updated dependencies [7bed083]
  - @sora-soft/framework@2.2.0
  - @sora-soft/database-component@2.2.0
  - @sora-soft/redis-component@2.2.0
  - @sora-soft/typia-decorator@2.2.0
  - @sora-soft/etcd-component@2.2.0
  - @sora-soft/etcd-discovery@2.2.0
  - @sora-soft/http-support@2.2.0

## 1.1.0

### Minor Changes

- a442c83: 修复 vite 编译报错的问题

### Patch Changes

- Updated dependencies [a442c83]
  - @sora-soft/database-component@2.1.0
  - @sora-soft/redis-component@2.1.0
  - @sora-soft/typia-decorator@2.1.0
  - @sora-soft/etcd-component@2.1.0
  - @sora-soft/etcd-discovery@2.1.0
  - @sora-soft/http-support@2.1.0
  - @sora-soft/framework@2.1.0

## 1.0.6

### Patch Changes

- Updated dependencies
  - @sora-soft/framework@2.0.4
  - @sora-soft/database-component@2.0.4
  - @sora-soft/etcd-component@2.0.4
  - @sora-soft/etcd-discovery@2.0.4
  - @sora-soft/http-support@2.0.5
  - @sora-soft/redis-component@2.0.4

## 1.0.5

### Patch Changes

- Updated dependencies [e658274]
- Updated dependencies [5adba31]
  - @sora-soft/framework@2.0.3
  - @sora-soft/database-component@2.0.3
  - @sora-soft/etcd-component@2.0.3
  - @sora-soft/etcd-discovery@2.0.3
  - @sora-soft/http-support@2.0.4
  - @sora-soft/redis-component@2.0.3

## 1.0.4

### Patch Changes

- Updated dependencies
  - @sora-soft/http-support@2.0.3

## 1.0.3

### Patch Changes

- ae7a0d7: 框架文档精细调整
- Updated dependencies [827dcd3]
- Updated dependencies [926d960]
- Updated dependencies [5ade32f]
- Updated dependencies [ae7a0d7]
- Updated dependencies [24c4d48]
  - @sora-soft/framework@2.0.2
  - @sora-soft/database-component@2.0.2
  - @sora-soft/redis-component@2.0.2
  - @sora-soft/etcd-component@2.0.2
  - @sora-soft/http-support@2.0.2
  - @sora-soft/etcd-discovery@2.0.2
  - @sora-soft/typia-decorator@2.0.2

## 1.0.2

### Patch Changes

- 6b5d1b9: 修改模板项目 tsconfig/eslint/Dockerfile
