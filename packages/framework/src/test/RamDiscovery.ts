...
import {Discovery, type IDiscoveryInfo} from '../lib/discovery/Discovery.js';
import type {Election} from '../lib/Election.js';
...
export class RamDiscovery extends Discovery {
  ...
  async getAllServiceList(): Promise<IServiceMetaData[]> {
    return [...this.serviceMap_].map(([_id, service]) => service);
  }
  ...
  async getAllEndpointList(): Promise<IListenerMetaData[]> {
    return [...this.listenerMap_].map(([_id, listener]) => listener);
  }
