...
### 需求:CLI 模板选择列表
（来源：sora-cli 的 `sora new` 命令）

`packages/sora-cli/src/commands/new.ts` 中的 `templates` 数组必须包含 `@sora-soft/base-cluster-template` 条目，描述为"带网关与业务服务的通用集群模板"。位置必须在 http-server-template 和 account-cluster-template 之间，形成复杂度递增的顺序。

#### 场景:用户选择模板时看到三个选项
- **当** 用户执行 `sora new my-app` 未指定模板参数
- **那么** 交互式列表显示三个模板选项，base-cluster-template 排在第二位

#### 场景:直接指定模板名创建
- **当** 用户执行 `sora new my-app @sora-soft/base-cluster-template`
- **那么** 跳过模板选择，直接下载 base-cluster-template 并创建项目
